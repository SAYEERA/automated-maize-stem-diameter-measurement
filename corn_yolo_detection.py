...
# ───── PROCESS EACH BATCH ────────────────────────────────
for batch_folder in sorted(all_batches):
    session_path = os.path.join(base_output_dir, batch_folder)
    blackbg_path = os.path.join(session_path, "picsart_blackbg", "jpg")
    ...
    if not image_files:
        print(f"⚠️ No images found in {blackbg_path}. Skipping YOLO for {batch_folder}.")
        continue
